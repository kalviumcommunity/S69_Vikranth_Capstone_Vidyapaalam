...
async function refreshToken(req, res) {
  ...
  try {
    ...
    // Ensure activeToken and refreshToken are selected
    const user = await User.findById(decoded.id).select('+refreshToken +activeToken +googleCalendar'); // Select googleCalendar for token refresh
    if (!user || user.refreshToken !== token) {
      console.log('Refresh Token Endpoint: User not found or refresh token mismatch.');
      // Clear tokens if mismatch found for security
      res.clearCookie('accessToken', cookieOptions);
    ...
    if (user.googleCalendar && user.googleCalendar.connected && user.googleCalendar.refreshToken) {
        ...
        try {
            ...
            // Optionally, mark calendar as disconnected if refresh fails
            user.googleCalendar.connected = false;
            user.googleCalendar.accessToken = null;
            user.googleCalendar.expiryDate = null;
            toast.error("Google Calendar connection lost. Please re-connect."); // Frontend toast
        }
    }
...
async function login(req, res) {
  try {
    ...
    res
      ...
      .json({
        message: 'Login successful',
        user: { id: user._id, name: user.name, email: user.email, googleCalendar: user.googleCalendar || { connected: false } } // Include googleCalendar
      });
