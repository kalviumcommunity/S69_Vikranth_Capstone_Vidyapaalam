...
function generateAccessToken(user) {
  // Ensure user.role is explicitly included, even if null/undefined initially
  const payload = { id: user._id, email: user.email, role: user.role || null };
...
// --- MODIFIED: googleAuthCallback for role, redirect, and calendar saving ---
async function googleAuthCallback(req, res) {
  const { code } = req.query;
...
async function refreshToken(req, res) {
  ...
  try {
    ...
    if (isBlacklisted) {
      console.log('Refresh Token Endpoint: Refresh token is blacklisted.');
      return res.status(401).json({ error: 'Refresh token revoked. Please log in again.' });
    }
    ...
    // Ensure activeToken, refreshToken, and googleCalendar are selected
    const user = await User.findById(decoded.id).select('+refreshToken +activeToken +googleCalendar');
    if (!user || user.refreshToken !== token) {
      console.log('Refresh Token Endpoint: User not found or refresh token mismatch.');
      res.clearCookie('accessToken', cookieOptions);
      res.clearCookie('refreshToken', cookieOptions);
    ...
    // If Google Calendar is connected and its access token is expired, try to refresh it
    if (user.googleCalendar && user.googleCalendar.connected && user.googleCalendar.refreshToken) {
        ...
        try {
            ...
            user.googleCalendar.expiryDate = new Date(credentials.expiry_date);
            console.log('Refresh Token Endpoint: Google Calendar access token refreshed.');
        } catch (calendarRefreshError) {
            console.error('Refresh Token Endpoint: Failed to refresh Google Calendar access token:', calendarRefreshError.message);
            // Mark calendar as disconnected if refresh fails
            user.googleCalendar.connected = false;
            user.googleCalendar.accessToken = null;
            user.googleCalendar.expiryDate = null;
            // Frontend toast should be handled by frontend after fetchUser
        }
    }
...
// --- MODIFIED: signup to include role and googleCalendar in response ---
async function signup(req, res) {
  try {
    const { name, email, password, role = 'student' } = req.body; // Default role for non-Google signup
    const normalizedEmail = email.trim().toLowerCase();
    ...
    res
      ...
      .json({
        ...
        user: { 
          ...
          googleCalendar: newUser.googleCalendar || { connected: false } // Include googleCalendar
        }
      });
...
// --- MODIFIED: login to include role and googleCalendar in response ---
async function login(req, res) {
  try {
    const { email, password } = req.body;
    ...
    res
      ...
      .json({
        ...
        user: { 
          ...
          googleCalendar: user.googleCalendar || { connected: false } // Include googleCalendar
        }
      });
    ...
    if (accessToken) {
        try {
            ...
            if (blacklistError.code === 11000) {
                console.warn('Logout: Access token already blacklisted (duplicate key error). Skipping re-blacklist.');
            } else {
    ...
    if (req.user && req.user.id) {
      ...
      try {
        ...
        if (user) {
          ...
          user.refreshToken = null;
          await user.save();
          console.log('Logout: Fallback: User active and refresh tokens cleared from DB via refresh token.');
        }
      } catch (err) {
        console.warn('Logout: Could not decode refresh token for user DB cleanup:', err.message);
      }
...
// --- MODIFIED: profile to return full googleCalendar object ---
async function profile(req, res) {
  try {
    const user = await User.findById(req.user.id)
    ...
    res.status(200).json({
      ...
      name: user.name,
      email: user.email,
      role: user.role || null, // Ensure role is always returned
      interestedSkills: user.interestedSkills,
