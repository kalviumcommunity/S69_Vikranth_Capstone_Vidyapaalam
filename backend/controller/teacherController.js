...
exports.createTeacherProfile = async (req, res) => {
  ...
  // Explicitly check for userId from auth middleware
  if (!userId) {
  ...
  try {
    ...
    // --- START FIX: Source fields from req.user if not provided in req.body ---
    name = name !== undefined ? name : user.name;
    email = email !== undefined ? email : user.email;
    phone = phone !== undefined ? phone : user.phoneNumber; // Assuming user.phoneNumber from your User model
    aboutMe = aboutMe !== undefined ? aboutMe : user.bio; // Assuming user.bio from your User model
    skills = Array.isArray(skills) && skills.length > 0 ? skills : (Array.isArray(user.skills) ? user.skills : []); // Assuming user.skills from your User model
    // --- END FIX ---
    ...
    const newTeacherProfile = new TeacherProfile({
      ...
      experience,
      hourlyRate,
      qualifications: Array.isArray(qualifications) ? qualifications : [], // Ensure qualifications is an array
