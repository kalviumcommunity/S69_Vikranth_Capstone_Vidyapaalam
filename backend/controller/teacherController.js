...
  return galleryPhotos.filter(photo => photo && photo.url && photo.publicId); // Corrected filter condition
};
...
exports.getTeacherProfileById = async (req, res) => {
  try {
    // Populate userId to get access to the availability array which is on the User model
    const profile = await TeacherProfile.findById(req.params.id).populate('userId', 'name bio availability teachingSkills');
    if (!profile) {
      return res.status(404).json({ message: 'Teacher not found' });
    }
...
    const formattedAvailability = profile.userId.availability.map(item => ({
      date: item.date.toISOString(), // Send as ISO string directly
      slots: item.slots.map(slot => ({
        startTime: slot.startTime,
        endTime: slot.endTime,
        available: slot.available, // Crucially, retain the boolean
      }))
    }));
...
      name: profile.userId.name || 'Unknown Teacher',
      avatarUrl: profile.avatar?.url || '',
      teachingSkills: profile.userId?.teachingSkills || profile.skills || [], // Use teachingSkills from User, fallback to TeacherProfile's skills
      rating: profile.rating || 0,
      hourlyRate: profile.hourlyRate || 0,
      bio: profile.userId.bio || 'No bio available',
      availability: formattedAvailability, // Send the correctly formatted array of objects
      videoUrl: profile.videoUrl?.url || '',
      galleryPhotos: profile.galleryPhotos || [],
    });
  } catch (err) {
    console.error('Error in getTeacherProfileById:', err);
    res.status(500).json({ error: err.message });
  }
};
...
    const availability = teacherProfile.userId.availability.map(item => ({
      date: item.date.toISOString(), // Changed to ISOString for full date/time precision
      slots: item.slots.map(slot => ({
        startTime: slot.startTime,
        endTime: slot.endTime,
        available: slot.available,
...
    res.status(200).json({ availability });
  } catch (err) {
    console.error('Error in getTeacherAvailability:', err);
    res.status(500).json({ error: err.message });
  }
};
...
    const user = teacherProfile.userId;
    ...
    const incomingDateOnly = new Date(date).toISOString().split('T')[0];

    const availabilityEntry = user.availability.find(a => a.date.toISOString().split('T')[0] === incomingDateOnly);
    if (!availabilityEntry) {
      return res.status(400).json({ message: 'No availability for the selected date' });
    }

    const slot = availabilityEntry.slots.find(s => {

      const generatedSlotId = `${availabilityEntry.date.toISOString().split('T')[0]}-${s.startTime}-${s.endTime}`;
      return generatedSlotId === slotId;
    });
...
    res.status(201).json({ message: 'Booking successful', bookedSlot: slot });
  } catch (err) {
    console.error('Error creating booking:', err); // Specific error log
    res.status(500).json({ error: err.message });
  }
};
