...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const bootstrapAuth = async () => {
      ...
      setLoading(true);
      try {
        let userData = await fetchUser(abortController.signal);
        if (userData) {
          console.log("Authenticated: User data fetched successfully.");
          return;
        }
        console.log("No current user data; attempting token refresh...");
        await api.post("/auth/refresh-token", {}, { withCredentials: true, signal: abortController.signal });
        console.log("Token refresh successful. Re-fetching user data with new token...");
        userData = await fetchUser(abortController.signal);
        if (!userData) {
          throw new Error("Failed to fetch user data after successful token refresh.");
        }
        console.log("User data re-fetched after successful refresh.");
        ...
        if (err.name === 'AbortError') {
          console.log("Auth bootstrap aborted due to component unmount.");
          return;
        }
        console.error("Authentication bootstrapping failed:", err.response?.status, err.message);
        if (isMountedRef.current) {
  ...
  const logout = async () => {
    try {
      ...
      console.error("Logout error:", err.response?.data || err.message || err.code);
      if (isMountedRef.current) {
        setUser(null);
        setLoading(false);
      }
    ...
    async (suffix, payload) => {
      try {
        ...
        if (!updatedUser) {
          ...
          if (!userData) {
            throw new Error(`Failed to fetch user data after updating ${suffix}`);
          }
          return userData;
        }
        if (isMountedRef.current && validateUserData(updatedUser)) {
        ...
        const userData = await fetchUser();
        if (!userData) {
          throw new Error(`Invalid user data after updating ${suffix}`);
        }
        return userData;
      } catch (err) {
        console.error(`Update profile ${suffix} failed:`, err.response?.status, err.response?.data || err.message);
