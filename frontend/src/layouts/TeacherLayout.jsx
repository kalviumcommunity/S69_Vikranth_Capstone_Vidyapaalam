...
import { useState, useEffect } from "react"; // Add useEffect for user name initialization
import { Outlet, Link, useLocation, useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import { Home, Star, Calendar, User, Settings, LogOut, Menu, X } from "lucide-react";
import {
...
export default function TeacherLayout() {
  ...
  const [displayedUserName, setDisplayedUserName] = useState("Guest"); // State to hold the displayed name
  const [avatarInitials, setAvatarInitials] = useState("??"); // State for avatar initials
  ...
  useEffect(() => {
    if (user && user.name) {
      ...
      setDisplayedUserName("Teacher"); // Default name if not logged in or name is missing
      setAvatarInitials("T"); // Default initials
    }
  }, [user]);
  ...
  const handleLogout = async () => {
    try {
      ...
      toast({ title: "Logged out successfully" });
      // The authLogout function already handles redirection, so no need to navigate here
    } catch (error) {
      ...
      <div className="flex min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 font-sans antialiased">
        ...
        >
          ...
          <div className="flex flex-col items-center px-6 py-6 border-b border-gray-100 pb-8 flex-shrink-0">
            ...
            {/* User Name and Role - Conditionally rendered */}
            {sidebarOpen && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex-1 flex flex-col relative md:static">
          ...
          <header className="flex items-center gap-4 p-4 bg-white border-b border-gray-200 shadow-sm sticky top-0 z-20 h-20">
            ...
            <h1 className="text-2xl font-bold text-gray-900 flex-grow truncate">{getPageTitle()}</h1>
            {/* The main avatar in the sidebar now handles the user identity, 
                so we removed the redundant avatar from the header here */}
          </header>
