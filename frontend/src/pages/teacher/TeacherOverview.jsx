...
    const studentName = session.studentId?.name || 'Unknown Student';
    ...
    // Function to create a unique callId from the two user IDs
    const getCallId = () => {
        const members = [user?.id, session.studentId?._id].sort();
        return members.join('-');
    };
...
          ) : (
            <>
              <Link // <-- CORRECTED: Changed button to Link
                to={`/call/${getCallId()}`} // <-- CORRECTED: Added video call link
