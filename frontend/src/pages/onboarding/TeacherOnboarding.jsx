...
// 		// Use location.search to get current URL search parameters
// 		const params = new URLSearchParams(location.search);
// 		const calendarAuthStatus = params.get('calendarAuthStatus');
// 		const calendarAuthError = params.get('error');
// 		const nextStepParam = params.get('nextStep'); // Get the new 'nextStep' parameter

// 		if (calendarAuthStatus) { // Check if any calendar auth status param exists
// 			if (calendarAuthStatus === 'success') {
// 				toast.success("Google Calendar connected successfully!");

// 				// If 'nextStep=availability' is present and onSetStep is a function, jump to that step
...
// 					onSetStep('availability'); // Call the prop to update parent's step state
// 				}
...
// 			newSearchParams.delete('nextStep'); // Remove the nextStep parameter as well
// 			navigate({ search: newSearchParams.toString() }, { replace: true });
// 		} 
// 	}, [authUser, authLoading, step, api, location.search, navigate, onSetStep]); // Added location.search, navigate, onSetStep to dependencies
                ...
                <div className="grid grid-cols-1 gap-2">
                  {timeSlots.map((slot) => (
                    <div
                      key={slot}
                      onClick={() => {
                        // Keep isLoading check for clicks, but not on the input value itself
