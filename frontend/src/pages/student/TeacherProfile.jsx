...
const TeacherProfile = () => {
  ...
  if (isLoading) {
    return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={{ duration: 0.4 }}
        className="space-y-8 px-4 py-6 bg-white"
      >
        <div className="flex flex-col md:flex-row items-start gap-6">
          <div className="h-32 w-32 bg-gray-200 rounded-full animate-pulse"></div>
          <div className="flex-1 space-y-3">
  ...
  if (!teacher) {
    return (
      <div className="text-center py-10 px-4">
        <h2 className="text-2xl font-semibold text-gray-800">Teacher not found</h2>
        <Link to="/student/favorites" className="mt-4 inline-block text-blue-600 hover:underline">
          Back to Favorites
        </Link>
      </div>
    );
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex flex-col md:flex-row items-start gap-6">
        <div className="h-32 w-32 rounded-full overflow-hidden ring-4 ring-orange-300 bg-gray-100">
          <img
            src={teacher.avatarUrl || "/placeholder.svg"}
            alt={teacher.name || "Unknown Teacher"}
            className="h-full w-full object-cover"
        ...
        <div className="flex-1 space-y-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">{teacher.name || "Unknown Teacher"}</h1>
            <div className="flex flex-wrap gap-2">
              <button className="flex items-center gap-1 text-orange-600 border border-orange-600 rounded-md px-3 py-1 text-sm hover:bg-orange-50">
                <Heart className="h-4 w-4" />
                Favorite
          ...
          <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{teacher.bio || "No bio available"}</p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.isArray(teacher.galleryPhotos) && teacher.galleryPhotos.length > 0 ? (
            ...
            <div className="text-center col-span-full text-gray-500">No gallery photos available</div>
          )}
        </CardContent>
      </Card>
      ...
      <div className="flex justify-center">
        <Link to={`/student/book-session/${teacher._id}`} className="w-full md:w-auto">
          <button className="bg-blue-600 text-white hover:bg-blue-700 flex items-center gap-2 w-full py-3 px-6 rounded-md text-sm font-medium">
            <Calendar className="h-5 w-5" />
            Book a Session
          </button>
        </Link>
